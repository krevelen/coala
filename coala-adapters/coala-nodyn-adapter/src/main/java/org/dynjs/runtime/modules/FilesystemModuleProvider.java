...
public class FilesystemModuleProvider extends ModuleProvider {
    ...
    protected File findFile(List<String> loadPaths, String moduleName) {
        ...
        for (String loadPath : loadPaths) {
            // require('foo');
            final File file = new File(loadPath, fileName);
            // foo.js is in the require path
            // System.err.println("Find module: "+file.getAbsolutePath());
            if (file.exists()) 
            {
            	loadPaths.add(file.getParent()); // needed for relative paths
            	return file;
            }
            
            final File indexFile = new File(loadPath + '/' + moduleName, "index.js");
            // System.err.println("Find module: "+indexFile.getAbsolutePath());
            if (indexFile.exists()) 
            {
            	loadPaths.add(indexFile.getParent()); // needed for relative paths
            	return indexFile;
            }
        }
        return null;
