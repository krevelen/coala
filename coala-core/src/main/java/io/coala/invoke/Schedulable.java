 ...
 * coala/service/scheduler/Schedulable.java $
...
public @interface Schedulable {
	...
	class Util {
		...
		/**
		 * @param reference
		 *            the method's annotated {@link Schedulable} value
		 * @param target
		 *            the object implementing the {@link Schedulable} method
		 * @param arguments
		 ...
		 *            the object implementing the {@link Schedulable} method
		 * @param arguments
		 *            the arguments to call the method with (or none or
		 *            {@code null})
		 ...
		 *            the object implementing the {@link Schedulable} method
		 * @param arguments
		 *            the arguments to call the method with (or {@code null})
		 * @throws CoalaException
		 */
		public static Object call(final String reference, final Object target,
		...
		private static Method findSchedulableMethod(final Class<?> superType,
			...
			String lookupName = superType.getName() + "." + reference;
			if (methodCache.containsKey(lookupName)) {
				return methodCache.get(lookupName);
			} else {
				for (Method method : superType.getDeclaredMethods()) {
					...
					if (annot != null && annot.value().equals(reference)) {
						methodCache.put(lookupName, method);
						return method;
					}
				}
			}
			methodCache.put(lookupName, null);
			return null;
		}
	}
}
